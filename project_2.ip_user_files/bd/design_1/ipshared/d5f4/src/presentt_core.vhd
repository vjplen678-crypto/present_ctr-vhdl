...
entity present_core is
    port (
        ...
        start_i   : in  std_logic;   -- start pulse (1 clk recommended)
        key_in    : in  std_logic_vector(79 downto 0);
        block_in  : in  std_logic_vector(63 downto 0);
        nonce_in   : in  std_logic_vector(63 downto 0); -- NONCE 64 bit

        block_out : out std_logic_vector(63 downto 0);
        done_o    : out std_logic    -- level, latched until next start
    );
end present_core;
...
architecture rtl of present_core is
    ...
    signal pt_reg    : std_logic_vector(63 downto 0); -- plaintext
        ...
        if rising_edge(clk) then
            if rstn = '0' then
                ...
                when IDLE =>
                    if start_i = '1' then
                        done_r    <= '0';
                        nonce_reg <= nonce_in;     -- ? L?U NONCE
                        pt_reg    <= block_in;     -- ? L?U PLAINTEXT
                        data_reg  <= nonce_in;     -- ? PRESENT(nonce)
                        key_reg   <= key_in;
                        round_cnt <= 1;
                        state     <= ROUND;
                    end if;
